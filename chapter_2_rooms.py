...
def random_room(data,loot,enemy):
    ...
    sleep(2)
    if nums == 1:#1
        print(f'Вы оглядываете комнату. Вы обнаружили {loot.get()}')
    elif nums == 1 and luck >= 2:
        print(f'Вы оглядываете комнату. Вы обнаружили {loot.get()}')
    elif nums == 1 and luck >= 3:
        print('Вы оглядываете комнату. В комнате обнаружен сундук. Попробуете его вскрыть? \
         Да(1)  Нет(2)' )
    elif nums == 2:#2
        print(f'Вы оглядываете комнату. Среди барахла Вы ничего не обнаружили...')
    elif nums == 3:#3
        print(f'Неожиданно на Вас нападает {enemy}')
    elif nums == 3 and luck >=3 or nums == 3 and dex >=8:
        res=input(f'Вы обнаружили врага {enemy} и он не заметил Вас первым.\n Можете напасть первым(1)\n \
        Или можете сбежать(2)')
        if res == '1':
            pass
            print(f'Одержав победу Вы забрли трофей {loot.get()}. Больше в этой комнате нечего делать...\
            Вы вышли из комнаты.')
        if res == '2':
            pass
        else:
